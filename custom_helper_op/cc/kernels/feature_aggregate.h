...
struct FeatureAggregateFunctor {
...
struct FeatureAggregateFunctor<Eigen::GpuDevice, T, half_centor> {
...
template <typename Device, typename T, bool half_centor>
struct FeatureAggregateGradFunctor {
  void operator()(    const Device& dev,
              const int64 batch_size, 
              const int64 image_height, 
              const int64 image_width,
...
// Partially specialize functor for GpuDevice.
template <typename T, bool half_centor>
struct FeatureAggregateGradFunctor<Eigen::GpuDevice, T, half_centor> {
  void operator()(    const Eigen::GpuDevice& dev,
              const int64 batch_size, 
              const int64 image_height, 
              const int64 image_width,
