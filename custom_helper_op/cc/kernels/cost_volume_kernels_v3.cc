...
using functor::CostVolumeV3Functor;
using functor::CostVolumeGradV3Functor;

template <typename Device, typename T>
class CostVolumeV3Op : public OpKernel {
  private:
  COST_REDUCE_METHOD reduce_method_;
  bool half_centor_;
  int32 groups_;
 public:
  explicit CostVolumeV3Op(OpKernelConstruction* ctx)
      : OpKernel(ctx) {
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    OP_REQUIRES(ctx, (image_channels / groups_) <= MAX_SUBCHANNELS_SIZE,
                errors::InvalidArgument("image_channels / groups_ less than MAX_SUBCHANNELS_SIZE(%d)", MAX_SUBCHANNELS_SIZE));
    ...
    if(reduce_method_ == COST_REDUCE_MEAN){
      ...
      OP_REQUIRES_OK(ctx, ctx->allocate_output(
                              1,
                              TensorShape({batch_size, image_height, image_width, image_depth, 3}),
                              &cost_mask));
    }


    if(half_centor_){
      CostVolumeV3Functor<Device, T, true>()(
                                    ctx->eigen_device<Device>(), reduce_method_,
                                    COST_FUNCTOR_ARG_LIST);
    } else {
      CostVolumeV3Functor<Device, T, false>()(
                                    ctx->eigen_device<Device>(), reduce_method_,
                                    COST_FUNCTOR_ARG_LIST);
    }
...
private:
  TF_DISALLOW_COPY_AND_ASSIGN(CostVolumeV3Op);
};
...
#define REGISTER(TYPE)                                              \
  REGISTER_KERNEL_BUILDER(Name("CostVolumeV3") \
                              .Device(DEVICE_GPU)                   \
                              .TypeConstraint<TYPE>("dtype"),        \
                          CostVolumeV3Op<GPUDevice, TYPE>)
...
template <typename Device, typename T>
class CostVolumeGradV3Op : public OpKernel {
private:
 COST_REDUCE_METHOD reduce_method_;
 bool half_centor_;
 int32 groups_;
 public:
  explicit CostVolumeGradV3Op(OpKernelConstruction* ctx)
      : OpKernel(ctx) {
    string reduce_method;
    OP_REQUIRES_OK(ctx, ctx->GetAttr("reduce_method", &reduce_method));
    if (reduce_method == "MEAN") {
  ...
  void Compute(OpKernelContext* ctx) override {
    ...
    OP_REQUIRES(ctx, (cost_grad.shape().dims() == 5) && (cost_grad.dim_size(0) == batch_size)
                      && (cost_grad.dim_size(1) == image_height) && (cost_grad.dim_size(2) == image_width) && (cost_mask.dim_size(3) == image_depth) && (cost_grad.dim_size(4) == 1),
                errors::InvalidArgument("cost_grad must have rank 5, and must compate to ref_image"));
    auto cost_mask_channels = (reduce_method_ == COST_REDUCE_MEAN)?1:3;
    ...
    if(half_centor_){
      CostVolumeGradV3Functor<Device, T, true>()(
                                    ctx->eigen_device<Device>(), reduce_method_,
                                    COST_FUNCTOR_GRAD_ARG_LIST);
    } else {
      CostVolumeGradV3Functor<Device, T, false>()(
                                    ctx->eigen_device<Device>(), reduce_method_,
                                    COST_FUNCTOR_GRAD_ARG_LIST);
    }
...
private:
  TF_DISALLOW_COPY_AND_ASSIGN(CostVolumeGradV3Op);
};
...
#define REGISTER(TYPE)                                              \
  REGISTER_KERNEL_BUILDER(Name("CostVolumeGradV3") \
                              .Device(DEVICE_GPU)                   \
                              .TypeConstraint<TYPE>("dtype"),        \
                          CostVolumeGradV3Op<GPUDevice, TYPE>)
