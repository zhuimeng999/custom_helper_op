...
from custom_helper_op import cost_volume, index_initializer
import numpy as np
...
def cost_aggregate_tfa(ref_image, src_images, base_plane, offsets, Rs, Ts, reduce_method="MEAN", half_centor=True):
    ...
    if reduce_method == "MEAN":
      cost = tf.reduce_mean(tf.square(maped_feature_volume - ref_image[:, None, :, :, None, :]), axis=1)
    else:
      cost = tf.reduce_min(tf.square(maped_feature_volume - ref_image[:, None, :, :, None, :]), axis=1)
...
class SparseConv3DTest(test.TestCase, parameterized.TestCase):
  ...
  def testCostVolumeSimple(self, BATCH_SIZE = 2, IMAGE_NUM = 2, IMAGE_HEIGHT = 5, IMAGE_WIDTH = 5, IMAGE_CHANNELS = 3, IMAGE_DEPTH = 4, reduce_method= "MEAN", half_centor=True):
    ...
    cost, cost_mask = cost_volume(batch_ref_image, batch_src_images, batch_ref_depth, batch_offsets, batch_Rs, batch_Ts, reduce_method=reduce_method, half_centor=half_centor)
    if reduce_method == "MEAN":
...
#       np.testing.assert_allclose(theoretical[2] , numerical[2], rtol=5e-5, atol=1e-6)
  ...
  def testCostVolumeGrad(self, BATCH_SIZE = 2, IMAGE_NUM = 2, IMAGE_HEIGHT = 5, IMAGE_WIDTH = 5, IMAGE_CHANNELS = 3, IMAGE_DEPTH = 4, reduce_method= "MEAN"):
    ...
    @tf.function
    def test_check(*args):
        cost, cost_mask = cost_volume(*args, reduce_method=reduce_method)
        if reduce_method == "MEAN":
            cost = tf.where(cost_mask > 0, cost, 0.)
        else:
            cost = tf.where(cost_mask >= 0, cost, 0.)
