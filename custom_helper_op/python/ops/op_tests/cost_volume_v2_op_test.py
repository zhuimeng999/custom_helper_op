...
def cost_aggregate_tfa(ref_image, src_images, base_plane, offsets, Rs, Ts, reduce_method="MEAN", half_centor=True):
    ...
    maped_feature_volume = tf.reshape(maped_feature_volume,
                                        (-1, src_num, image_shape[0], image_shape[1], max_d, image_shape[2]))
    cost = tf.reduce_sum(tf.reshape(maped_feature_volume*ref_image[:, None, :, :, None, :], (-1, src_num, image_shape[0], image_shape[1], max_d, 3, image_shape[2]//3)), axis=-1)
