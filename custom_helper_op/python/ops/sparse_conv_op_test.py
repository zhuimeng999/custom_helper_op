...
from custom_helper_op import sparse_conv2d, sparse_conv3d
import numpy as np
...
class SparseConv3DTest(test.TestCase, parameterized.TestCase):
    ...
    def testForward(self, BATCH_SIZE, IMAGE_HEIGHT, IMAGE_WIDTH, IMAGE_DEPTH, VIRTUAL_DEPTH, IN_CHANNELS, OUT_CHANNELS, KERNEL_SIZE):
        ...
        start = time.time()
        res_nn = tf.nn.conv3d(images_nn, filters, strides=(1, 1, 1, 1, 1), padding="VALID", dilations=(1, 1, 1, 1, 1))
        nn_time = time.time() - start
    ...
    #     my_forward_time = time.time() - start
    #     start = time.time()
    #     grad = tape.gradient(res, [images, filters, default_value, base_plane])
    #     my_backforward_time = time.time() - start
    #     # print(res.numpy()[0, 1, 1, 0], tf.reduce_sum(filters[0, :, :, :]*images[0, :KERNEL_SIZE[0], :KERNEL_SIZE[1], :]).numpy())
    #     images_nn = tf.pad(tf.constant(images, dtype=images.dtype), [[0, 0], [KERNEL_SIZE[0]//2, KERNEL_SIZE[0]//2], [KERNEL_SIZE[1]//2, KERNEL_SIZE[1]//2], [0, 0]],
    #                                                     mode="CONSTANT", constant_values=default_value)
    ...
    #   (2, 4, 6, 5, 8, 2, 1, (5, 3, 3)),
    # )
    # def testGradientFloat64(self, BATCH_SIZE, IMAGE_HEIGHT, IMAGE_WIDTH, IMAGE_DEPTH, VIRTUAL_DEPTH, IN_CHANNELS, OUT_CHANNELS, KERNEL_SIZE):
    #     images = np.random.random([BATCH_SIZE, IMAGE_HEIGHT, IMAGE_WIDTH, IMAGE_DEPTH, IN_CHANNELS])
    #     filters = np.random.random([OUT_CHANNELS, KERNEL_SIZE[0], KERNEL_SIZE[1], KERNEL_SIZE[2], IN_CHANNELS])
    #     base_plane = np.random.randint(0, high=5, size=[BATCH_SIZE, IMAGE_HEIGHT, IMAGE_WIDTH, 1], dtype=np.int32)
    #     default_value = tf.constant(0, dtype=images.dtype)

    #     @tf.function
    #     def test_check(*args):
    #         cost = sparse_conv3d(*args, base_plane)
    #         return tf.reduce_mean(cost)
    #     with self.cached_session():
    #         # res = sparse_conv2d(images, filters, base_plane, default_value, offsets)
